...
def save(embeddings):
    dictionary = dict([(embeddings.index_to_key[i], i) for i in range(
        len(embeddings.index_to_key))])
    reverse_dictionary = dict(zip(dictionary.values(), dictionary.keys()))  # 把dictionary的键和值反过来
