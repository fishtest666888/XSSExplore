...
def pre_process():
    ...
    xssed_data = []
    normal_data = []
    with open("./data/xssed_deduplicate.csv", "r", encoding="utf-8", errors='ignore') as f:
        reader = csv.DictReader(f, fieldnames=["payload"])
        for row in reader:
            payload = row["payload"]
            word = GeneSeg(payload)
            xssed_data.append(word)
    with open("./data/dmzo_nomal.csv", "r", encoding="utf-8") as f:
        reader = csv.DictReader(f, fieldnames=["payload"])
        for row in reader:
            payload = row["payload"]
            word = GeneSeg(payload)
...
if __name__ == "__main__":
    ...
    accuracy = accuracy_score(test_labels, pre)
    f1 = f1_score(test_labels, pre)
